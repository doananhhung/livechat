...
import { VisitorSessionMetadata, WebSocketEvent, ConversationUpdatedPayload, NavigationEntry } from '@live-chat/shared-types'; // Import shared types
...
describe('Realtime Chat Architecture (E2E)', () => {
  ...
  const connectAndIdentify = async (visitorUid: string) => {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      setTimeout(() => reject(new Error('Sockets did not connect in time')), 10000);
    });
  ...
  it('should store sessionMetadata on first sendMessage', async () => {
    ...
    const metadata: VisitorSessionMetadata = {
      ...
      urlHistory: [
        { url: 'http://localhost.com/lp', title: 'Landing Page', timestamp: new Date().toISOString() },
        { url: 'http://localhost.com/page1', title: 'Page 1', timestamp: new Date().toISOString() },
      ],
    };
  ...
  it('should append URL to history and emit conversationUpdated on updateContext', async () => {
    ...
    const initialMetadata: VisitorSessionMetadata = {
      ...
      urlHistory: [
        { url: 'http://localhost.com/initial', title: 'Initial Page', timestamp: new Date().toISOString() },
      ],
    };
    ...
    agentSocket.on(WebSocketEvent.CONVERSATION_UPDATED, (payload: ConversationUpdatedPayload) => {
    ...
    visitorSocket.emit('updateContext', { currentUrl: newUrl, currentTitle: newTitle }); // Pass title for consistency
    ...
    if (conversationUpdatedPayload) { // Check for null before accessing fields
  ...
  it('should respect MAX_URL_HISTORY_LENGTH on updateContext', async () => {
    ...
    for (let i = 0; i < MAX_URL_HISTORY_LENGTH; i++) {
      initialHistory.push({ url: `http://localhost.com/page-${i}`, title: `Page ${i}`, timestamp: new Date().toISOString() });
    }
    const initialMetadata: VisitorSessionMetadata = {
      referrer: 'https://initial.com',
      landingPage: 'http://localhost.com/initial',
    ...
    visitorSocket.emit('updateContext', { currentUrl: newUrl, currentTitle: newTitle });
    await new Promise(resolve => setTimeout(resolve, 1000));
    ...
    expect(conversation).toBeDefined();
    expect(conversation?.metadata?.urlHistory).toHaveLength(MAX_URL_HISTORY_LENGTH);
    // The oldest entry should have been removed
    expect(conversation?.metadata?.urlHistory[0].url).toBe('http://localhost.com/page-1');
    // The new entry should be the last one
    expect(conversation?.metadata?.urlHistory[MAX_URL_HISTORY_LENGTH - 1].url).toBe(newUrl);
  });
});